fn main() {
  ...
  let connection = gudp::Bind::new(src_port).connect(dst_port);
...
fn listen(conn: gudp::Connection<std::net::UdpSocket>, buf: &mut [u8]) {
  loop {
...
fn ping(conn: gudp::Connection<std::net::UdpSocket>, buf: &mut [u8]) {
  loop {
